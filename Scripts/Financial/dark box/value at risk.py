...
#result plot
plt.hist(historical_x_day_returns_dollar*100, bins=50, density=True, alpha=0.5, label=f'{days}-Day Returns')
color_map = cm.get_cmap('plasma')  
for i, (cl, VaR) in enumerate(zip(confidence_levels, VaRs)):
  color = color_map(i / len(confidence_levels))  
  plt.axvline(x=VaR*100, linestyle='--', color=color, label=f'VaR at {int(cl * 100)}% Confidence = {round(VaR*100,2)}')
